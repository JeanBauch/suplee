  ...
  <div>
    <div v-if="callToAction" class="relative w-full">
      ...
      <span class="cursor-pointer" @click="$emit('searchProduct', nameProductToSearch)">
        <div class="absolute z-10 w-1/6 lg:w-1/5 h-full rounded-lg top-0 right-0 bg-primary-green-medium group before:content-[''] before:absolute before:top-0 before:-left-8 before:z-[-1] before:h-full before:w-[60px] before:rounded-tl-full before:rounded-bl-full before:bg-primary-green-medium">
          <SearchIcon class="absolute top-3 right-4 lg:right-6 w-14 h-14 text-complement-background-white z-10 transition-transform duration-300 group-hover:scale-125" />
        </div>
      </span>
    </div>
    <div v-else-if="productListing" class="relative w-full">
      ...
      >
      <span class="cursor-pointer" @click="$emit('searchProduct', nameProductToSearch)">
        <div class="absolute z-10 w-20 h-full rounded-lg top-0 right-0 bg-primary-green-medium group">
          <SearchIcon class="absolute top-2 right-5 w-10 h-10 text-complement-background-white z-10 transition-transform duration-300 group-hover:scale-125" />
        </div>
      </span>
    </div>
